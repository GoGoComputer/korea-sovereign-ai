...
### 🪟 "Docker requires privileged access" password prompt appears
On macOS login (especially **right after disabling Rosetta**), Docker Desktop auto-starts and shows:

> 🔒 Docker — *Docker Desktop requires privileged access to configure privileged port mapping. Enter your password to allow this.*

**This repo (korea-ai) does not use Docker at all.** The prompt comes from Docker Desktop itself, not from anything here. What to do:

- **If you need Docker** (for other work) → enter your macOS login password → OK. Normal.
- **If you don't use Docker** → click Cancel, then disable auto-start:
  ```bash
  # 1) Whale icon in menu bar → Settings → General →
  #    uncheck "Start Docker Desktop when you sign in"
  # 2) Or quit / uninstall entirely
  osascript -e 'quit app "Docker"'
  brew uninstall --cask docker        # full removal
  ```
- To stop being asked every time → Settings → Advanced →
  turn off *"Allow privileged port mapping"* or use *"Configure..."* to grant once.

> ❓ Why does it appear after disabling Rosetta? When Rosetta turns off, Docker rebuilds its virtualization backend (VM networking) and re-requests permission for sub-1024 "privileged port" mappings. This is expected behavior.

#### Then a "Finish setting up Docker Desktop" window appears — what to pick?

After the password prompt (or after an update) you may see:
...
- **If you only use this repo (korea-ai)**: pick anything and click Finish. This repo doesn't use Docker. (To stop seeing this dialog at all, disable Docker auto-start — see above.)
- **If you also use Docker**: **Use recommended settings → Finish** (the default). It picks settings that work for most developers; you can change them later in Settings.
- Choose **Use advanced settings** only if you want to manually decide:
  1. Whether to allow privileged-port (<1024) mapping
  2. Whether to install the docker CLI symlink in `/usr/local/bin`
  3. Whether to create the `/var/run/docker.sock` socket
  If unsure, just go with recommended.

> 📝 "requires password" on the recommended option just means it needs your macOS login password once more to apply the privileged-port permission you saw earlier. Enter it and you're done.
